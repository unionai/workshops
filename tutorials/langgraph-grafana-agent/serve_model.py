"""The model factory's other half: serve an open model on a Union app, OpenAI-compatible.

Step 4 points the same agent at this endpoint with `vllm:qwen3-8b`. vLLM speaks the
OpenAI chat-completions API, so the only thing that changes in the agent is the model
string; LangChain's `ChatOpenAI` does the rest.
...
APP_NAME = os.environ.get("VLLM_APP_NAME", "oncall-qwen3-8b")
...
if __name__ == "__main__":
    ...
    print(f"App: {app.url}")
    print(f"Put this in .env:  VLLM_BASE_URL={app.url}/v1")
    print(
        f'Then:              flyte run step4_model_factory.py bakeoff --models \'["anthropic:claude-opus-5", "vllm:{MODEL_ID}"]\''
    )
