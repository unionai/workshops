"""Gradio frontend for fraud scoring API. Local: python demo.py | Remote: API_URL=... python demo.py | Deploy: flyte deploy demo.py demo_env"""

import os
import logging
import requests
import gradio as gr
import uvicorn
from fastapi import FastAPI

import flyte
from flyte.app import Parameter
from flyte.app.extras import FastAPIAppEnvironment

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

API_URL_ENV = "API_URL"
CATEGORIES = [
    "entertainment", "food_dining", "gas_transport", "grocery_net",
    "grocery_pos", "health_fitness", "home", "kids_pets",
    "misc_net", "misc_pos", "personal_care", "shopping_net",
    "shopping_pos", "travel",
]


def score_transaction(user_id, amt, category, merch_lat, merch_long, hour, day_of_week):
    """Call the scoring API and format the response."""
    api_url = os.environ.get(API_URL_ENV, "http://localhost:8080")
    logger.info(f"Calling {api_url}/score with user_id={user_id}, amt={amt}, category={category}")
    try:
        resp = requests.get(
            f"{api_url}/score",
            params={"user_id": int(user_id), "amt": float(amt), "category": category, "merch_lat": float(merch_lat), "merch_long": float(merch_long), "hour": int(hour), "day_of_week": int(day_of_week)},
            timeout=60,
        )
        resp.raise_for_status()
        data = resp.json()
        logger.info(f"Got response: {data.get('fraud_prediction')} ({data.get('fraud_probability')})")
    except requests.ConnectionError as e:
        logger.error(f"Connection error: {e}")
        return f"**Connection Error** — Cannot reach `{api_url}`\n\nIs the scoring app running?", "", "", ""
    except requests.Timeout:
        logger.error("Request timed out (60s)")
        return f"**Timeout** — Scoring app at `{api_url}` did not respond within 60s", "", "", ""
    except Exception as e:
        logger.error(f"Error: {e}")
        return f"**Error:** {e}", "", "", ""

    prob, risk, prediction = data["fraud_probability"], data["risk_level"], data["fraud_prediction"]
    icons = {"HIGH": "🚨", "MEDIUM": "⚠️", "LOW": "✅"}
    verdict = f"## {icons.get(risk, '✅')} {prediction} ({risk} RISK)\n\nFraud probability: **{prob:.1%}**"

    signals = data.get("signals", [])
    signals_md = "\n".join(f"- {s}" for s in signals) if signals else "No risk signals detected."

    profile = data.get("user_profile", {})
    profile_md = f"- **Transactions:** {profile.get('txn_count', 'N/A')}\n- **Avg amount:** ${profile.get('mean_amt', 0):.2f}\n- **Std dev:** ${profile.get('std_amt', 0):.2f}\n- **Age:** {profile.get('age', 'N/A')}"

    txn, scoring = data.get("transaction", {}), data.get("scoring", {})
    model_prob = data.get("model_probability", prob)
    txn_md = (
        f"- **Amount:** ${txn.get('amt', amt):.2f}\n- **Category:** {txn.get('category', category)}\n"
        f"- **Hour (UTC):** {txn.get('hour', 'N/A')}\n- **Day of week:** {txn.get('day_of_week', 'N/A')}\n"
        f"- **Amount z-score:** {scoring.get('amt_zscore', 'N/A')}\n- **Distance from home:** {scoring.get('distance_from_home', 'N/A')} mi\n"
        f"- **Model probability:** {model_prob:.1%}" + (f"\n- **Rule override applied**" if scoring.get("rule_applied") else "")
    )
    return verdict, signals_md, profile_md, txn_md


def build_gradio_app():
    with gr.Blocks(title="Fraud Detector", theme=gr.themes.Soft()) as blocks:
        gr.Markdown("# Fraud Detection Demo\nScore transactions against the ML model + Feast feature store.")
        with gr.Row():
            with gr.Column(scale=1):
                gr.Markdown("### Transaction Details")
                user_id = gr.Number(label="User ID", value=42, precision=0)
                amt = gr.Number(label="Amount ($)", value=25.00)
                category = gr.Dropdown(label="Merchant Category", choices=CATEGORIES, value="grocery_pos")
                merch_lat = gr.Number(label="Merchant Latitude", value=33.9)
                merch_long = gr.Number(label="Merchant Longitude", value=-80.3)
                hour = gr.Slider(label="Hour (UTC)", minimum=0, maximum=23, step=1, value=22)
                day_of_week = gr.Dropdown(label="Day of Week", choices=[(d, i) for i, d in enumerate(["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"])], value=3)
                score_btn = gr.Button("Score Transaction", variant="primary")
            with gr.Column(scale=1):
                gr.Markdown("### Results")
                verdict = gr.Markdown(label="Prediction", value="*Click Score to analyze a transaction.*")
                signals = gr.Markdown(label="Risk Signals")
                with gr.Accordion("User Profile (from Feast)", open=False):
                    profile = gr.Markdown()
                with gr.Accordion("Transaction Info", open=False):
                    txn_info = gr.Markdown()
        score_btn.click(fn=score_transaction, inputs=[user_id, amt, category, merch_lat, merch_long, hour, day_of_week], outputs=[verdict, signals, profile, txn_info], show_progress="minimal")
        gr.Markdown("---")
        gr.Markdown(
            "### Quick Tests\n| Scenario | User ID | Amount | Category | Lat | Long | Hour | Day |\n|----------|---------|--------|----------|-----|------|------|-----|\n| Normal grocery | 42 | $25 | grocery_pos | 33.9 | -80.3 | 14 | Wed |\n| Late-night large | 42 | $2,500 | shopping_net | 33.9 | -80.3 | 2 | Wed |\n| Far-away | 42 | $100 | travel | 48.8 | 2.3 | 22 | Thu |\n| Suspicious | 42 | $9,999 | shopping_net | 48.8 | 2.3 | 23 | Thu |"
        )
    return blocks


def create_app():
    fastapi_app = FastAPI(title="Fraud Detection Demo")
    @fastapi_app.get("/health")
    async def health():
        api_url = os.environ.get(API_URL_ENV, "http://localhost:8080")
        try:
            resp = requests.get(f"{api_url}/health", timeout=10)
            return {"status": "healthy", "api_url": api_url, "scorer_status": resp.json()}
        except Exception as e:
            return {"status": "unhealthy", "api_url": api_url, "error": str(e)}
    gr.mount_gradio_app(fastapi_app, build_gradio_app(), path="/")
    return fastapi_app


_bare_app = FastAPI(title="Fraud Detection Demo")
demo_env = FastAPIAppEnvironment(
    name="fraud-demo",
    app=_bare_app,
    description="Interactive Gradio UI for fraud detection scoring",
    parameters=[Parameter(name="api_url", value="https://sparkling-dew-5ccf3.apps.demo.hosted.unionai.cloud", env_var=API_URL_ENV)],
    uvicorn_config=uvicorn.Config("demo:create_app", factory=True, port=8080),
    image=flyte.Image.from_debian_base().with_pip_packages("gradio", "requests", "fastapi", "uvicorn"),
    resources=flyte.Resources(cpu=1, memory="1Gi"),
    requires_auth=False,
)


if __name__ == "__main__":
    api_url = os.environ.get(API_URL_ENV, "http://localhost:8080")
    print(f"Scoring API: {api_url}\nMake sure the scoring app is running (python app.py)")
    build_gradio_app().launch()
